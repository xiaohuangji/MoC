...
Training-memory, training-throughput, PPL, and decode benchmarks use C4 input batches. Place the dataset under:
...
The C4 folders should contain the raw `json.gz` shards. A local copy or symbolic link of the `t5-base` tokenizer should be placed under:
...
The single-layer FFN latency benchmark uses fixed-shape hidden-state tensors because it measures the FFN kernel itself rather than the data pipeline.
...
The PPL benchmark trains a selected preset with the C4 training schedule in [configs](configs), then evaluates validation perplexity with shifted causal-LM loss and padding labels ignored.
