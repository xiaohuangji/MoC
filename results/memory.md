...
Memory is measured with `torch.cuda.max_memory_allocated()`. The script first runs one warmup AdamW step on a C4 batch to initialize optimizer state, then resets CUDA peak-memory stats and measures one full `forward + backward + AdamW step`. Reported values use decimal GB. This is not `nvidia-smi` process memory and not `torch.cuda.max_memory_reserved()`.
...
JSON outputs are written under `results/raw/memory/` by default.
